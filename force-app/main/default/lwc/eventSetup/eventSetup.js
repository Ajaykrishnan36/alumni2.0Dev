...
export default class EventSetup extends LightningElement {
    aiStarUrl = aiStar;
    _eventData = {};
    @api
    get eventData() {
    ...
    syncRichTextDom() {
        ...
        const descriptionDiv = this.template.querySelector('#description');
        const agendaDiv = this.template.querySelector('#agenda');

        if (descriptionDiv && this.descriptionHtml !== undefined) {
            const isFocused = this.template.activeElement === descriptionDiv;
            if (!isFocused && (this.isRichTextEmpty(descriptionDiv) || descriptionDiv.innerHTML !== this.descriptionHtml)) {
                descriptionDiv.innerHTML = this.descriptionHtml;
            }
        }
        if (agendaDiv && this.agendaHtml !== undefined) {
            const isFocused = this.template.activeElement === agendaDiv;
            if (!isFocused && (this.isRichTextEmpty(agendaDiv) || agendaDiv.innerHTML !== this.agendaHtml)) {
                agendaDiv.innerHTML = this.agendaHtml;
            }
        }
    }

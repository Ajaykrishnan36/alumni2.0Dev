...
import checkGuestInvitePermission from "@salesforce/apex/KenPortalEventController.checkGuestInvitePermission";
import canBulkRegisterGroups from "@salesforce/apex/KenPortalEventController.canBulkRegisterGroups";
import previewGroupRegistrationForEvent from "@salesforce/apex/KenPortalEventController.previewGroupRegistrationForEvent";
...
export default class KenEventRegistrationPage extends NavigationMixin(
  LightningElement
) {
  ...
  // Bulk (group) registration is events-admin-only — hidden entirely for everyone else.
  @track canBulkRegister = false;
  @track participants = [];
  @track guestName = "";
  @track guestEmail = "";
  @track guestEmailError = "";
  @track guestPhone = "";
  // Best-effort typing-time hint only — result.matched/allowed from checkGuestInvitePermission.
  // The actual gate is always re-checked fresh in handleAddGuest, never read from this cache.
  @track guestSuggestion = null;
  _guestEmailDebounce = null;
  @track searchQuery = "";
  ...
  connectedCallback() {
    ...
    getPortalConfigs()
      .then((cfg) => {
        this.feeDisabled = cfg?.disableEventFee === true;
      })
      .catch(() => {});

    canBulkRegisterGroups()
      .then((result) => {
        this.canBulkRegister = result === true;
      })
      .catch(() => {
        this.canBulkRegister = false;
      });
  }
  ...
  disconnectedCallback() {
    ...
    if (this._guestEmailDebounce) clearTimeout(this._guestEmailDebounce);
  }

  _isSurveySubmitted(participantId) {
    try {
      const arr = JSON.parse(
        sessionStorage.getItem("kenFbSubmitted:" + this._recordId) || "[]"
      );
  ...
  _loadQuestionnaire() {
    ...
    getQuestionnaireByEventId({ eventId: rid })
      .then((result) => {
        // getQuestionnaireByEventId returns a QuestionnaireDTO, not an SObject.
        const qId = result && result.questionSetId;
        if (qId) {
  ...
  _buildDietaryOptions(currentValue) {
    const raw =
      this.eventData && this.eventData.dietaryPreferences
        ? this.eventData.dietaryPreferences
        : "";
  ...
  _processSchedules(data) {
    ...
    this.sessions = sorted.map((s) => ({
      ...
      // Format from the instant so the session time follows the viewer's
      // device timezone. _msToTime renders a bare milliseconds-from-midnight
      // value that carries no zone — fallback only, for pre-migration rows.
      startTime: s.startInstant
        ? formatTime(s.startInstant)
        : s.startTime != null
          ? this._msToTime(s.startTime)
          : null,
      endTime: s.endInstant
        ? formatTime(s.endInstant)
        : s.endTime != null
          ? this._msToTime(s.endTime)
          : null,
  ...
  get showSessionFooterInfo() {
    return this.currentStep >= 2;
  }

  get progressStyle() {
    const pct =
      this.currentStep === 1 ? 33.33 : this.currentStep === 2 ? 66.66 : 100;
    return `width: ${pct}%`;
  }
  ...
  get eventTitle() {
    return this.eventData
      ? this.eventData.name || this.eventData.title || ""
      : "";
  }
  get eventDateRange() {
    return this.eventData
      ? this.eventData.dateRange || this.eventData.startDate || ""
      : "";
  }
  get eventLocation() {
    return this.eventData
      ? this.eventData.location || this.eventData.venueName || ""
      : "";
  }
  get eventLanguage() {
    return this.eventData ? this.eventData.language || "" : "";
  }
  ...
  get myselfBtnClass() {
    return this.isMyselfMode ? "mode-btn active" : "mode-btn";
  }
  get bulkBtnClass() {
    return this.isBulkMode ? "mode-btn active" : "mode-btn";
  }

  // -- Meal/dietary getters --
  get showMealsColumn() {
    return !!(this.eventData && this.eventData.offerMeals);
  }
  get showDietaryColumn() {
    return !!(this.eventData && this.eventData.collectDietary);
  }
  get hasSurvey() {
  ...
  get showFeeLabel() {
    return (
      this.showFees &&
      !!(this.eventData && !this.eventData.noFee && this.sessions.length > 0)
    );
  }
  ...
  get totalPages() {
    return Math.max(
      1,
      Math.ceil(this.filteredParticipants.length / this.pageSize)
    );
  }
  ...
  get showPagination() {
    return this.filteredParticipants.length > this.pageSize;
  }
  get isPrevDisabled() {
  ...
  get hasSelectedSessions() {
    return this.selectedSessions.length > 0;
  }

  get totalAmount() {
    const sessionSum = this.selectedSessions.reduce(
      (sum, s) => sum + (s.price || 0),
      0
    );
  ...
  get pricePerSession() {
    ...
    const total = this.selectedSessions.reduce(
      (sum, s) => sum + (s.price || 0),
      0
    );
  ...
  handleGuestEmailChange(e) {
    ...
    this.guestEmailError =
      v && !emailRe.test(v) ? "Please enter a valid email address" : "";
    this.guestSuggestion = null;

    if (this._guestEmailDebounce) clearTimeout(this._guestEmailDebounce);
    if (!v || !emailRe.test(v)) return;
    // eslint-disable-next-line @lwc/lwc/no-async-operation
    this._guestEmailDebounce = setTimeout(() => {
      checkGuestInvitePermission({ email: v })
        .then((result) => {
          // Email may have changed while this was in flight — ignore a stale result.
          if ((this.guestEmail || "").trim() !== v) return;
          this.guestSuggestion = result && result.matched ? result : null;
        })
        .catch(() => {
  ...
  // Shown greyed-out with a hover tooltip when blocked — never hidden — so the person
  // adding a guest can see the match exists and why it can't be added, same treatment
  // as the Group invite search results.
  get guestSuggestionText() {
    if (!this.guestSuggestion || !this.guestSuggestion.matched) return "";
    if (this.guestSuggestion.allowed === false) {
      return `Matches existing alumni: ${this.guestSuggestion.matchedName} (invite not allowed)`;
    }
    return `Matches existing alumni: ${this.guestSuggestion.matchedName}`;
  }
  get guestSuggestionClass() {
    if (!this.guestSuggestion || this.guestSuggestion.allowed === false)
      return "field-hint field-hint-blocked";
    return "field-hint";
  }
  get guestSuggestionTitle() {
    if (this.guestSuggestion && this.guestSuggestion.allowed === false) {
      return (
        this.guestSuggestion.reason ||
        "This alumni does not accept invites from you."
      );
    }
    return "";
  }
  handleGuestPhoneChange(e) {
    this.guestPhone =
      e.detail?.e164 || e.detail?.national || e.target?.value || "";
  }

  get maxGuests() {
    const n =
      this.eventData && this.eventData.guestCount != null
        ? Number(this.eventData.guestCount)
        : 0;
    return n > 0 ? n : 5;
  }
  ...
  handleAddGuest() {
    ...
    this.guestEmailError = "";
    // The guest phone field renders the country picker but nothing ever checked
    // what was typed into it, so a guest could be added with a two-digit number.
    // Optional, so an empty value still passes.
    const guestPhoneCheck = validatePhoneNumber(this.guestPhone, null, false);
    if (!guestPhoneCheck.valid) {
      this._showToast("Error", guestPhoneCheck.message, "error");
      return;
    }
    const guestCount = this.participants.filter(
      (p) => p.type === "guest"
    ).length;
    if (guestCount >= this.maxGuests) {
      this._showToast(
        "Error",
        `Maximum ${this.maxGuests} guests can be added.`,
        "error"
      );
      return;
    }

    // Always re-check fresh against whatever is currently in the field — never rely on
    // the typing-time suggestion cache, which can be stale, skipped, or bypassed (paste).
    checkGuestInvitePermission({ email: emailVal })
      .then((result) => {
        if (result && result.allowed === false) {
          this.guestEmailError =
            result.reason || "This person cannot be invited to this event.";
          return;
        }
        this._pushGuestParticipant();
      })
      .catch(() => {
        // Fail-open: don't block a legitimate add if the permission check itself errors.
        this._pushGuestParticipant();
      });
  }

  _pushGuestParticipant() {
  ...
  _toggleGroup(id) {
    ...
    if (didSelect) {
      // Recompute the real, preference-filtered headcount — never trust the group's
      // raw member count for pricing/attendee totals.
      previewGroupRegistrationForEvent({ groupId: id })
        .then((preview) => {
          const rowId = `group_${id}`;
          const allowed =
            preview && preview.allowedMembers != null
              ? preview.allowedMembers
              : 0;
          const blocked =
            preview && preview.blockedMembers != null
              ? preview.blockedMembers
              : 0;
          const total =
            preview && preview.totalMembers != null
          ...
          this.participants = this.participants.map((p) => {
            ...
            return {
              ...p,
              memberCount: allowed,
              phone: `${allowed} members`,
              previewText:
                blocked > 0
                  ? `${allowed} of ${total} members will be registered — ${blocked} member(s)' preferences don't allow this invite`
                  : ""
            };
          });
          this._saveRegistrationDraft();
        ...
        .catch(() => {
          /* keep the optimistic raw count if the preview call fails */
        });
    }
  }
  ...
  _restoreRegistrationDraft() {
    try {
      ...
      if (draft.registrationType)
        this.registrationType = draft.registrationType;
      ...
      this.groups = (this.groups || []).map((g) => ({
        ...g,
        selected: restoredGroupIds.has(g.id)
      }));
      return true;
    } catch (e) {
      return false;
  ...
  handleMealsToggle(e) {
    ...
    this.participants = this.participants.map((p) => {
      ...
      return {
        ...p,
        meals: checked,
        dietaryPref: checked ? p.dietaryPref : ""
      };
    });
    this._saveRegistrationDraft();
  }
  ...
  _toggleSession(id) {
    ...
    this.sessions = this.sessions.map((s) => {
      if (s.id !== id) return s;
      return { ...s, checked: newSet.has(s.id) };
    });
    this._buildDateGroups();
  }
    ...
    if (this.currentStep === 1) {
      ...
      if (this.eventData && this.eventData.collectDietary) {
        ...
        if (missingDietary) {
          this._showToast(
            "Error",
            "Please select a dietary preference for each participant who has opted for meals.",
            "error"
          );
          return;
        }
  ...
  handleFinalRegister() {
    ...
    saveEventRegistrations({
      ...
      .catch((err) => {
        this._showToast(
          "Error",
          err.body ? err.body.message : "Registration failed.",
          "error"
        );
  ...
  closeModal() {
    if (this.embeddedModal === true || this.embeddedModal === "true") {
      this.dispatchEvent(
        new CustomEvent("closeregistration", { bubbles: true })
      );
      return;
    }
    const rid = this._recordId;
